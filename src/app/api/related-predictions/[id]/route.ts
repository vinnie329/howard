...
import { getRelatedPredictions } from '@/lib/data';
...
export async function GET(
  ...
  try {
    const results = await getRelatedPredictions(id, limit, threshold);
    ...
    console.error('Related predictions error:', err instanceof Error ? err.message : err);
    return NextResponse.json({ results: [] });
  }
}
